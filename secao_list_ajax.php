...
if (empty($con->getErrorMessage())) {
            ...
            $sql = "SELECT sec_id, sec_titulo, sec_conteudo, sec_da_criacao, sec_ativo FROM section";
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
      $texto = strip_tags($row['sec_conteudo']);
      $resumo = mb_strlen($texto) > 70 ? mb_substr($texto, 0, 70) . '...' : $texto;       

        echo '<tr id="' . $row['sec_id'] . '">';  
        echo '<td>';
        echo '<img src="eye.png" alt="" class="view classPointer" title="Visualizar section" />'; 
        echo '&emsp;<img src="edit.webp" alt="" class="edit classPointer" title="Editar" />';       
        echo '</td>';          
        echo '<td>' . $row['sec_titulo'] . '</td>';
        echo '<td>' . $resumo . '</td>';        
        ...
        echo '</label>'; 
        echo '</td>';                
        echo '<td>' . date('d/m/Y H:i:s', strtotime($row['sec_da_criacao'])) . '</td>';        
        echo '</tr>';
    }
